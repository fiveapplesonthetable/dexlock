...
/// `dexlock race`: report inconsistent-locking (data-race) findings as JSON.
pub fn run_race(inputs: &[PathBuf], scope: Option<&str>, out: &Path) -> Result<usize> {
    let t0 = Instant::now();
    let dex = input::parse_inputs(inputs, scope).context("parsing dex inputs")?;
    let races = dex::race::races(&dex);
    log::info!("found {} inconsistently-locked fields in {:.2?}", races.len(), t0.elapsed());
    ...
    struct RaceJson<'a> {
        field: &'a str,
        guard: &'a str,
        guarded_accesses: usize,
        unguarded: Vec<AccJson<'a>>,
    }
